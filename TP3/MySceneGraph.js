...
class MySceneGraph {
    ...
    parseSpritesheets(spritesheetsNode) {
        ...
        if (children.length === 0) { //Check for no texture
            return "[spritesheets] no spritesheets defined"
        }
            ...
            for (let j = 0; j < descendantsNodes.length; j++) {
                if (descendantsNodes[j].nodeName === "noderef") {
                    ...
                    if (type === "rectangle") {
                        ...
                        const x = this.reader.getString(descendantsNodes[j], 'x');
                        const y = this.reader.getString(descendantsNodes[j], 'y');
                        const z = this.reader.getString(descendantsNodes[j], 'z');
                        if (x == null || y ==null|| z ==null)
                            return "[nodes] Missing values for board leaf. Node id: " + nodeID;

                        if ( isNaN(x) || isNaN(y)|| isNaN(z)) {
                            return "[nodes] Wrong values for board leaf. Node ID: " + nodeID;
                        }
                        this.board= new MyBoard(this.scene,x,y,z);
                        descendants.push({
                            type: "board",
                            board: this.board
                        });
