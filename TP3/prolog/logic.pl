...
% wants to move
validateCapture(_,_,GameState,FinalMoveGameState,PieceAndMove,Check,LenghtMove,_) :-
    Check =:= LenghtMove +1,
    FinalMoveGameState='"invalid"'.
...
% wants to move
validateContent(_, _, GameState,FinalMoveGameState,PieceAndMove,LenghtMove,LenghtMove,ChosenPiece) :-
    ChosenPiece='"invalid"'.
...
checkWinner(GameState,Winner):-
    ...
    (
        BlackPoints@<WhitePoints,Winner='WHITES';
        WhitePoints@<BlackPoints,Winner='BLACKS';
        (
            write('TIED POINTS!!!!\nChecking for higher stack...\n'),
            (
              (BlackMaxLength@<WhiteMaxLength,Winner='WHITES');
              (WhiteMaxLength@<BlackMaxLength,Winner='BLACKS');
                (Winner='TIED')
            )
